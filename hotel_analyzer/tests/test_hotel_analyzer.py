...
dict_for_test_basic_df = {'Name': ['Inturist', 'Rodina', 'Hotel 4', 'Moskva', 'Ladoga'],
               ...
               'Longitude': ['-111.403390', '-111.403300', '-111.403490', '-7ff6.532350', '-76.53550']}
...
def create_test_temperature_df():
    date_index = pd.date_range(date.today() - timedelta(days=2), date.today() + timedelta(days=1), freq="d")
    return pd.DataFrame(dict_for_test_temperature_df, index=date_index)


def test_clean_df(create_test_basic_df):
    df = clean_df(create_test_basic_df)
    dict_for_compare_df = {'Name': ['Inturist', 'Rodina', 'Ladoga'],
               'Country': ['RU', 'RU', 'RU'],
               'City': ['Moscow', 'Moscow', 'Saint-Petersburg'],
               'Latitude': [40.91089, 40.91071, 49.802666],
               'Longitude': [-111.40339, -111.4033, -76.5355]}

    assert_frame_equal(pd.DataFrame(dict_for_compare_df).reset_index(drop=True), df.reset_index(drop=True), check_dtype=False)



def test_top_cities(create_test_basic_df):
    df = get_series_of_top_cities(create_test_basic_df).reset_index()
    dict_for_compare_df = {'Country': ['RU'], 'City': ['Moscow_RU']}
    assert_frame_equal(pd.DataFrame(dict_for_compare_df).reset_index(drop=True), df.reset_index(drop=True), check_dtype=False)


def test_get_center_coords(create_test_basic_df):
    df = clean_df(create_test_basic_df)
...
def test_get_day_city_max_temp(create_test_temperature_df):
    assert get_day_city_max_temp(create_test_temperature_df) == ('2021-01-25', 'Mos', 14)


def test_get_max_change(create_test_temperature_df):
    assert get_max_change(create_test_temperature_df) == ('Mos', 8)


def test_get_day_city_min_temp(create_test_temperature_df):
    assert get_day_city_min_temp(create_test_temperature_df) == ('2021-01-22', 'Mo', 1)


def test_get_max_dif(create_test_temperature_df):
